...
namespace ofxCv {
    ...
    void transformMyPoint(Point2f& src, Point2f& dst, Mat& transform) {
        Mat Msrc=Mat::zeros(3, 1, CV_64FC1);//DataType<float>::type); //Mat(3,1,CV_32FC1,0); // a 3x1 vector (homogeneous coordinates for the src point)
        Mat Mdst=Mat::zeros(3, 1, CV_64FC1);//DataType<float>::type); //(3,1,CV_32FC1,0); // a 3x1 vector (homogeneous coordinates for the dst point)
        // load the source point/
        Msrc.at<double>(0,0)=src.x;
        Msrc.at<double>(1,0)=src.y;
        Msrc.at<double>(2,0)=1.0; // homogeneous coordinate value for a point (not at infinity)
        // then, perform the product of this point by the transform, and save it in dst:
        Mdst=transform*Msrc;
        // Assign the transformed values to X and Y: 
        //cout << "until here all well..." << endl;
        dst.x=Mdst.at<double>(0,0)/Mdst.at<double>(2,0); 
        dst.y=Mdst.at<double>(1,0)/Mdst.at<double>(2,0); 
    }
